...
## Отчёт AI-консультанта

Отчёт содержит `answer`, `strengths`, `risks`, `consequences`, `recommendations`
и `follow_up_question`. Поле `consequences` обязательно в новых ответах AI:
массив до пяти блоков `{"title": "...", "explanation": "..."}`, допускающий `[]`.
Последствия объясняют ожидаемый результат решений отдельно от рисков и рекомендаций.
Ответ без поля или с невалидными блоками отклоняется как `invalid_ai_response`.

Backend сохраняет последствия в JSON отчёта и включает их в текст истории,
передаваемой консультанту при следующем вопросе. Сценарный API возвращает поле
`consequences` и при отправке вопроса, и внутри `report` в истории чата.
Для старых записей БД, у которых поле отсутствует, чтение возвращает `[]`:
последствия для исторических ответов не генерируются. Это совместимость чтения
JSON, поэтому миграция схемы БД не нужна.

## Потоковый консультант песочницы

Фронтенд обращается только к backend: `POST /api/v1/sandbox/v2/chat/stream`
(совместимость v1: `/api/v1/sandbox/chat/stream`). Backend пересылает
`{message, history, context}` на фиксированный маршрут ИИ-сервиса, заданного
переменной `AI_SERVICE_URL` в backend/.env. История сценарной Астаны хранится
в БД; история локальной песочницы пока передаётся браузером.

SSE передаётся без буферизации. Отмена запроса закрывает upstream; ошибки
подключения возвращаются как JSON, обрыв начатого потока — как SSE error.
Используются общий лимит размера запроса и лимит частоты AI-чата. Для песочницы
лимит общий для v1/v2 по IP подключения (за reverse proxy IP может быть общим).
