...
const DISTRICTS: { id: DistrictId; name: string; coordinates: [number, number]; focus: [number, number]; osmRelation: number }[] = [
  { id: "saryarka", name: "Сарыарка", coordinates: [71.3226517, 51.1988462], focus: [71.370, 51.190], osmRelation: 3486954 },
  { id: "baikonur", name: "Байконур", coordinates: [71.4580973, 51.2264690], focus: [71.455, 51.192], osmRelation: 8593081 },
  { id: "almaty", name: "Алматы", coordinates: [71.5473172, 51.1550554], focus: [71.505, 51.158], osmRelation: 3482819 },
  { id: "nura", name: "Нура", coordinates: [71.3160362, 51.1008961], focus: [71.365, 51.116], osmRelation: 20593940 },
  { id: "esil", name: "Есиль", coordinates: [71.4106032, 51.0592468], focus: [71.4305, 51.1283], osmRelation: 3479876 },
];
...
function focusDistrict(map: MapLibreMap, district: (typeof DISTRICTS)[number]) {
  map.flyTo({ center: district.focus, zoom: 14.25, pitch: 58, bearing: -24, duration: 900, essential: true });
}

function showCenter(map: MapLibreMap) {
  map.flyTo({ center: [71.4305, 51.1283], zoom: 15.3, pitch: 62, bearing: -24, duration: 1000, essential: true });
}
...
function addDistrictBoundaries(map: MapLibreMap, selectedDistrict: DistrictId) {
  ...
  map.addLayer({
    ...
    filter: ["==", ["get", "id"], selectedDistrict],
...
export function AstanaMap({ selectedDistrict, onSelectDistrict, onFallback }: {
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => setStatus((current) => current === "loading" ? "error" : current), 16000);
    ...
    map.on("load", () => {
      ...
      if (map.getLayer("astana-district-fill")) {
        map.on("click", "astana-district-fill", (event) => {
          const district = DISTRICTS.find((item) => item.id === event.features?.[0]?.properties?.id);
          if (!district) return;
          if (selectionRef.current === district.id) focusDistrict(map, district);
  ...
  useEffect(() => {
    ...
    if (map?.getLayer("astana-selected-fill")) {
      map.setFilter("astana-selected-fill", ["==", ["get", "id"], selectedDistrict]);
      map.setFilter("astana-selected-outline", ["==", ["get", "id"], selectedDistrict]);
    }
  ...
  return <div className="astana-map-shell" role="region" aria-label="Реальная карта Астаны с пятью районами из задания">
    ...
    <div className="astana-map-label"><span className="astana-map-live" /> <strong>АСТАНА · РЕАЛЬНАЯ КАРТА</strong><small>OSM / OpenFreeMap · границы районов · 3D-здания</small></div>
    {status === "ready" && <div className="astana-map-controls"><button type="button" onClick={() => mapRef.current && showCity(mapRef.current, true)}>Весь город</button><button type="button" onClick={() => mapRef.current && showCenter(mapRef.current)}>3D центр</button><span>Районы · улицы · здания</span></div>}
    {status === "loading" && <div className="astana-map-state" role="status"><strong>Загружаем карту Астаны</strong><span>Дороги, река и здания приходят из открытых картографических данных.</span></div>}
    {status === "error" && <div className="astana-map-state is-error" role="alert"><strong>Карта временно недоступна</strong><span>Для онлайн-карты нужен интернет. Схема районов работает без него.</span><button type="button" onClick={onFallback}>Открыть схему</button></div>}
  </div>;
}
